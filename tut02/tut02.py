...
def process_tcp_3_way_handshake_start(packet):
    print(packet)
    filename="TCP_3_Way_Handshake_Start_2001CS66.pcap"
...
def process_tcp_handshake_close(packet):
    ...
    if(packet[scapy.TCP].flags==0x011 or x<3):
        print(packet)
        filename="TCP_Handshake_Close_2001CS66.pcap"
        scapy.wrpcap(filename, packet, append=True)
        x-=1
        if(x==0):
            return True
...
def process_arp_request_response(packet):
    print(packet)
    filename="ARP_Request_Response_2001CS66.pcap"
    scapy.wrpcap(filename, packet, append=True)
...
def process_dns_request_response(packet):
    global x
    filename="DNS_Request_Response_2001CS66.pcap"
    if(x<2 and packet.haslayer(scapy.DNSQR) and packet[scapy.DNSQR].qtype==1 and ("icici" in str(packet[scapy.DNSQR].qname))): # change fifa
        print(packet)
        scapy.wrpcap(filename, packet, append=True)
        x=x+1
...
def process_ping_request_response(packet):
    print(packet)
    filename="PING_Request_Response_2001CS66.pcap"
    scapy.wrpcap(filename, packet, append=True)
...
def process_ftp_connection_start(packet):
    print(packet)
    filename="FTP_Connection_Start_2001CS66.pcap"
    scapy.wrpcap(filename, packet, append=True)
...
def process_ftp_connection_close(packet):
    ...
    if(packet[scapy.TCP].flags==0x011 or x<3):
        print(packet)
        filename="FTP_Connection_Close_2001CS66.pcap"
        scapy.wrpcap(filename, packet, append=True)
        x-=1
        if(x==0):
            return True
