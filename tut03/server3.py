...
def bind_socket():
    try:
        ...
        server.listen(0)
        inputs_from_client.append(server)
    except socket.error as message:
...
def SELECT():
    while inputs_from_client:
        readable, writable, exceptional = select.select(
            inputs_from_client, outputs_to_clients, inputs_from_client)
        ...
        for s in readable:  
            if s is server:  
                ...
                inputs_from_client.append(connection)
